...
try:
    ...
    st.warning("Không tìm thấy BaseFeatures.fdef của RDKit. Một số tính năng liên kết hydro có thể bị thiếu.")
...
def rdkit_feature_row_single(smiles, compute_3d=True):
    """Tính toàn bộ features cho 1 SMILES duy nhất"""
    try:
...
if btn and smiles_input:
    ...
    if not model:
        ...
        with st.spinner("Đang tính toán đặc trưng (gồm 3D shape)..."):
            full_feats_dict = rdkit_feature_row_single(smiles_input, compute_3d=COMPUTE_3D)
        ...
        if full_feats_dict:
            ...
            try:
                ...
                with col2:
                    ...
                    if mol: st.image(Draw.MolToImage(mol), width=300)
                    ...
                    if missing_feats:
                        st.warning(f"⚠️ Cảnh báo: Có {len(missing_feats)} đặc trưng thiếu (đã điền 0). Kết quả có thể không chính xác tuyệt đối.")
                        with st.expander("Xem các đặc trưng thiếu"):
